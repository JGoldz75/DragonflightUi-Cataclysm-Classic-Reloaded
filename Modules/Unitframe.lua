...
local optionsPlayer = {
    ...
    args = {
        ...
        fadingBars = {
            type = 'toggle',
            name = L["UnitFrameFadingBars"],
            desc = L["UnitFrameFadingBarsDesc"] .. getDefaultStr('fadingBars', 'player'),
            group = 'headerStyling',
            order = 8.5,
            new = true,
            editmode = true
...
local optionsTarget = {
    ...
    args = {
        ...
        fadingBars = {
            type = 'toggle',
            name = L["UnitFrameFadingBars"],
            desc = L["UnitFrameFadingBarsDesc"] .. getDefaultStr('fadingBars', 'target'),
            group = 'headerStyling',
            order = 8.5,
            new = true,
            editmode = true
        ...
        hitIndicators = {
            type = 'toggle',
            name = L["TargetFrameHitIndicators"],
            desc = L["TargetFrameHitIndicatorsDesc"] .. getDefaultStr('hitIndicators', 'target'),
            group = 'headerStyling',
            order = 8.6,
            new = true,
            editmode = true
        },
        targetDebuffsMineFirst = {
            type = 'toggle',
            name = L["TargetFrameDebuffsMineFirst"],
            desc = L["TargetFrameDebuffsMineFirstDesc"] .. getDefaultStr('targetDebuffsMineFirst', 'target'),
            group = 'headerStyling',
            order = 8.7,
            new = true,
            editmode = true
        ...
        targetDebuffSize = {
            ...
            desc = L["TargetFrameDebuffSizeDesc"] .. getDefaultStr('targetDebuffSize', 'target'),
            min = 0,
            max = 50,
            bigStep = 1,
            group = 'headerStyling',
        ...
        targetDebuffPersonalScale = {
            type = 'range',
            name = L["TargetFrameDebuffPersonalScale"],
            desc = L["TargetFrameDebuffPersonalScaleDesc"] .. getDefaultStr('targetDebuffPersonalScale', 'target'),
            min = 1,
            max = 2,
            bigStep = 0.05,
            group = 'headerStyling',
        ...
        targetDebuffOffsetX = {
            type = 'range',
            name = L["TargetFrameDebuffOffsetX"],
            desc = L["TargetFrameDebuffOffsetXDesc"] .. getDefaultStr('targetDebuffOffsetX', 'target'),
            min = -100,
            max = 100,
            bigStep = 1,
            group = 'headerStyling',
        ...
        targetDebuffOffsetY = {
            type = 'range',
            name = L["TargetFrameDebuffOffsetY"],
            desc = L["TargetFrameDebuffOffsetYDesc"] .. getDefaultStr('targetDebuffOffsetY', 'target'),
            min = -100,
            max = 100,
            bigStep = 1,
            group = 'headerStyling',
        ...
        targetDebuffSpacing = {
            type = 'range',
            name = L["TargetFrameDebuffSpacing"],
            desc = L["TargetFrameDebuffSpacingDesc"] .. getDefaultStr('targetDebuffSpacing', 'target'),
            min = -12,
            max = 20,
            bigStep = 1,
            group = 'headerStyling',
...
local optionsPet = {
    ...
    args = {
        ...
        fadingBars = {
            type = 'toggle',
            name = L["UnitFrameFadingBars"],
            desc = L["UnitFrameFadingBarsDesc"] .. getDefaultStr('fadingBars', 'pet'),
            group = 'headerStyling',
            order = 9.5,
            new = true,
            editmode = true
...
local optionsFocus = {
    ...
    args = {
        ...
        fadingBars = {
            type = 'toggle',
            name = L["UnitFrameFadingBars"],
            desc = L["UnitFrameFadingBarsDesc"] .. getDefaultStr('fadingBars', 'focus'),
            group = 'headerStyling',
            order = 8.5,
            new = true,
            editmode = true
...
local optionsParty = {
    ...
    args = {
        ...
        fadingBars = {
            type = 'toggle',
            name = L["UnitFrameFadingBars"],
            desc = L["UnitFrameFadingBarsDesc"] .. getDefaultStr('fadingBars', 'party'),
            group = 'headerStyling',
            order = 8.5,
            new = true,
            editmode = true
...
function Module:ApplySettings(sub)
    ...
    do
        ...
        Module.SetupFadingBarsForUnit(PlayerFrameHealthBar, PlayerFrameManaBar, obj.fadingBars)
    ...
    do
        ...
        Module.SetupFadingBarsForUnit(TargetFrameHealthBar, TargetFrameManaBar, obj.fadingBars)
    ...
    do
        ...
        Module.SetupFadingBarsForUnit(PetFrameHealthBar, PetFrameManaBar, obj.fadingBars)
    ...
    if DF.Wrath then
        ...
        do
            ...
            Module.SetupFadingBarsForUnit(FocusFrameHealthBar, FocusFrameManaBar, obj.fadingBars)
...
local function DFFadingBar_OnUpdate(self, elapsed)
    ...
    if not bar or not bar.DFFadingBarEnabled or not bar:IsShown() then
        self:Hide()
        return
    end

    self.DFElapsed = (self.DFElapsed or 0) + elapsed
    local progress = math.min(self.DFElapsed / FADING_BAR_DURATION, 1)
    local value = self.DFStartValue + ((self.DFTargetValue - self.DFStartValue) * progress)

    self:SetValue(value)
    self:SetAlpha(0.65 * (1 - progress))

    if progress >= 1 then self:Hide() end
end

function Module.SyncFadingBar(statusBar)
    local trail = statusBar and statusBar.DFFadingBar
    if not trail then return end

    local texture = statusBar:GetStatusBarTexture()
    if texture and texture.GetTexture then
        trail:SetStatusBarTexture(texture:GetTexture() or "Interface\\TargetingFrame\\UI-StatusBar")
    end

    local r, g, b, a = 1, 1, 1, 1
    if statusBar.GetStatusBarColor then
        r, g, b, a = statusBar:GetStatusBarColor()
    elseif texture and texture.GetVertexColor then
        r, g, b, a = texture:GetVertexColor()
    end
    trail:SetStatusBarColor(r or 1, g or 1, b or 1, a or 1)

    local minValue, maxValue = statusBar:GetMinMaxValues()
    trail:SetMinMaxValues(minValue, maxValue)

    if statusBar.DFMask and not trail.DFMaskAdded then
        trail:GetStatusBarTexture():AddMaskTexture(statusBar.DFMask)
        trail.DFMaskAdded = true
    end
end

function Module.FadingBarOnValueChanged(statusBar, value)
    local trail = statusBar.DFFadingBar
    if not trail then return end

    local oldValue = statusBar.DFFadingBarLastValue or value
    statusBar.DFFadingBarLastValue = value

    if not statusBar.DFFadingBarEnabled then
        trail:Hide()
        return
    end

    local minValue, maxValue = statusBar:GetMinMaxValues()
    if maxValue <= minValue or value >= oldValue then return end

    Module.SyncFadingBar(statusBar)

    local startValue = oldValue
    if trail:IsShown() then startValue = math.max(trail:GetValue(), oldValue) end

    trail.DFElapsed = 0
    trail.DFStartValue = startValue
    trail.DFTargetValue = value
    trail:SetValue(startValue)
    trail:SetAlpha(0.65)
    trail:Show()
end

function Module.SetupFadingBar(statusBar, enabled)
    if not statusBar then return end

    if not statusBar.DFFadingBar then
        local parent = statusBar:GetParent() or statusBar
        local trail = CreateFrame('StatusBar', nil, parent)
        trail:SetPoint('TOPLEFT', statusBar, 'TOPLEFT', 0, 0)
        trail:SetPoint('BOTTOMRIGHT', statusBar, 'BOTTOMRIGHT', 0, 0)
        trail:SetFrameStrata(statusBar:GetFrameStrata())
        trail:SetFrameLevel(math.max(0, statusBar:GetFrameLevel() - 1))
        trail.DFStatusBar = statusBar
        trail:SetScript('OnUpdate', DFFadingBar_OnUpdate)
        trail:Hide()

        statusBar.DFFadingBar = trail
        statusBar.DFFadingBarLastValue = statusBar:GetValue()
        statusBar:HookScript('OnValueChanged', Module.FadingBarOnValueChanged)
        statusBar:HookScript('OnSizeChanged', function(self)
            if not self.DFFadingBar then return end
            self.DFFadingBar:ClearAllPoints()
            self.DFFadingBar:SetPoint('TOPLEFT', self, 'TOPLEFT', 0, 0)
            self.DFFadingBar:SetPoint('BOTTOMRIGHT', self, 'BOTTOMRIGHT', 0, 0)
        end)
    end

    statusBar.DFFadingBarEnabled = enabled and true or false
    statusBar.DFFadingBarLastValue = statusBar:GetValue()
    Module.SyncFadingBar(statusBar)

    if not enabled then statusBar.DFFadingBar:Hide() end
end

function Module.SetupFadingBarsForUnit(healthBar, powerBar, enabled)
    Module.SetupFadingBar(healthBar, enabled)
    Module.SetupFadingBar(powerBar, enabled)
end

function Module.ResetFadingBar(statusBar)
    if not statusBar then return end

    statusBar.DFFadingBarLastValue = statusBar:GetValue()
    if statusBar.DFFadingBar then statusBar.DFFadingBar:Hide() end
end

function Module.ResetFadingBarsForUnit(healthBar, powerBar)
    Module.ResetFadingBar(healthBar)
...
function Module.UpdateTargetDebuffLayout(targetFrame)
    ...
    local settings = Module.db and Module.db.profile.target
...
local function DFHitIndicator_OnUpdate(self, elapsed)
    self.DFElapsed = (self.DFElapsed or 0) + elapsed
    local duration = self.DFDuration or HIT_INDICATOR_DURATION
    local progress = self.DFElapsed / duration
    if progress >= 1 then
        self:Hide()
        return
    end
    ...
    self:ClearAllPoints()
    self:SetPoint('CENTER', self.DFAnchor, 'CENTER', self.DFStartX + self.DFMoveX * progress,
                  self.DFStartY + self.DFMoveY * progress)

    local fadeStart = self.DFFadeStart or 0.35
    local maxAlpha = self.DFMaxAlpha or 1
    if progress > fadeStart then
        self:SetAlpha(maxAlpha * (1 - ((progress - fadeStart) / (1 - fadeStart))))
    else
        self:SetAlpha(maxAlpha)
    end
end

local function createHitIndicator(parent)
    local hit = CreateFrame('Frame', nil, parent)
    hit:SetSize(86, 24)
    hit:SetFrameStrata('HIGH')
    hit:SetFrameLevel(parent:GetFrameLevel() + 20)
    hit.Text = hit:CreateFontString(nil, 'OVERLAY', 'NumberFontNormal')
    hit.Text:SetPoint('CENTER', hit, 'CENTER', 0, 0)
    hit:SetScript('OnUpdate', DFHitIndicator_OnUpdate)
    hit:Hide()
    return hit
end

function Module.ShowHitIndicator(parent, anchor, amount, critical)
    if not parent or not anchor or not amount or amount <= 0 then return end

    parent.DFHitIndicatorPool = parent.DFHitIndicatorPool or {}

    local hit
    for _, candidate in ipairs(parent.DFHitIndicatorPool) do
    ...
    if not hit then
        hit = createHitIndicator(parent)
        table.insert(parent.DFHitIndicatorPool, hit)
    end

    local amountText = amount
    if amount >= 100000 and AbbreviateLargeNumbers then
        amountText = AbbreviateLargeNumbers(amount)
    elseif BreakUpLargeNumbers then
        amountText = BreakUpLargeNumbers(amount)
    end

    hit.Text:SetText(amountText)
    hit.Text:SetTextColor(1, critical and 0.25 or 0.82, critical and 0.1 or 0.25, 1)
    hit.Text:SetScale(critical and 1.3 or 0.72)
    hit.DFAnchor = anchor
    hit.DFElapsed = 0
    hit.DFDuration = critical and HIT_INDICATOR_CRIT_DURATION or HIT_INDICATOR_DURATION
    hit.DFFadeStart = critical and 0.55 or 0.25
    hit.DFMaxAlpha = critical and 1 or 0.55
    hit.DFStartX = critical and random(-8, 8) or random(-4, 4)
    hit.DFStartY = critical and random(-4, 6) or random(-2, 3)
    hit.DFMoveX = critical and random(-12, 12) or random(-5, 5)
    hit.DFMoveY = critical and random(24, 38) or random(10, 18)
    hit:SetAlpha(hit.DFMaxAlpha)
    hit:ClearAllPoints()
    hit:SetPoint('CENTER', anchor, 'CENTER', hit.DFStartX, hit.DFStartY)
    hit:Show()
end

function Module.GetHitIndicatorTarget(destGUID)
    if not destGUID then return nil end

    local function check(unit, unitFrame, portrait)
        if unitFrame and portrait and unitFrame:IsShown() and UnitExists(unit) and UnitGUID(unit) == destGUID and
            UnitCanAttack('player', unit) then
            return unitFrame, portrait
        end
    end

    local parent, anchor = check('target', TargetFrame, TargetFramePortrait)
    if parent then return parent, anchor end

    if DF.Wrath then
        parent, anchor = check('focus', FocusFrame, FocusFramePortrait)
        if parent then return parent, anchor end
    end

    parent, anchor = check('targettarget', TargetFrameToT, TargetFrameToTPortrait)
    if parent then return parent, anchor end

    if DF.Wrath then
        parent, anchor = check('focustarget', FocusFrameToT, FocusFrameToTPortrait)
        if parent then return parent, anchor end
    end

    for i = 1, 4 do
        local bossFrame = _G['DragonflightUIBoss' .. i .. 'Frame']
        if bossFrame then
            parent, anchor = check('boss' .. i, bossFrame, bossFrame.Portrait)
            if parent then return parent, anchor end
        end
    end
end

local function isGroupDamageSource(sourceFlags)
    local band = bit and bit.band
    local affiliationMask = (COMBATLOG_OBJECT_AFFILIATION_MINE or 0) + (COMBATLOG_OBJECT_AFFILIATION_PARTY or 0) +
                                (COMBATLOG_OBJECT_AFFILIATION_RAID or 0)

    if band and sourceFlags and affiliationMask > 0 then return band(sourceFlags, affiliationMask) ~= 0 end
...
function Module.HandleCombatLogEvent(...)
    local db = Module.db and Module.db.profile
    if not db or not db.target.hitIndicators then return end

    local timestamp, subEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName,
          destFlags, destRaidFlags, arg12, arg13, arg14, arg15, arg16, arg17, arg18, arg19, arg20, arg21 =
        getCombatLogEventInfo(...)

    if not isGroupDamageSource(sourceFlags) then return end

    local amount, critical
    if subEvent == 'SWING_DAMAGE' then
        amount = arg12
        critical = arg18
    elseif subEvent == 'RANGE_DAMAGE' or subEvent == 'SPELL_DAMAGE' or subEvent == 'SPELL_PERIODIC_DAMAGE' or
        subEvent == 'DAMAGE_SHIELD' or subEvent == 'DAMAGE_SPLIT' then
        amount = arg15
        critical = arg21
    end
    ...
    local parent, anchor = Module.GetHitIndicatorTarget(destGUID)
    if parent and anchor then Module.ShowHitIndicator(parent, anchor, amount, critical) end
end
...
function Module.UpdatePlayerFrameHealthBar()
    ...
    Module.SyncFadingBar(PlayerFrameHealthBar)
end
...
function Module.HookVertexColor()
    ...
    local updateTargetFrameHealthBar = function()
        ...
        Module.SyncFadingBar(TargetFrameHealthBar)
    end
    ...
    if DF.Wrath then
        local updateFocusFrameHealthBar = function()
            ...
            Module.SyncFadingBar(FocusFrameHealthBar)
        end
...
function Module.ChangeTargetFrame()
    ...
    if not TargetFrame.DFDebuffLayoutHooked then
        TargetFrame.DFDebuffLayoutHooked = true
        hooksecurefunc('TargetFrame_UpdateAuras', function(self)
            Module.UpdateTargetDebuffLayout(self)
        end)
    end
end
...
function Module.ReApplyTargetFrame()
    ...
    Module.SyncFadingBar(TargetFrameHealthBar)
    Module.SyncFadingBar(TargetFrameManaBar)
...
function Module.ReApplyFocusFrame()
    ...
    Module.SyncFadingBar(FocusFrameHealthBar)
    Module.SyncFadingBar(FocusFrameManaBar)
...
function Module.ChangePetFrame()
    ...
    frame.UpdatePetManaBarTexture = function()
        ...
        PetFrameManaBar:GetStatusBarTexture():SetVertexColor(1, 1, 1, 1)
        Module.SyncFadingBar(PetFrameManaBar)
    end
...
function Module:UpdatePartyState(state)
    ...
    for i = 1, 4 do
        local pf = _G['PartyMemberFrame' .. i]
        Module.SetupFadingBarsForUnit(_G['PartyMemberFrame' .. i .. 'HealthBar'],
                                      _G['PartyMemberFrame' .. i .. 'ManaBar'], state.fadingBars)
        Module.UpdatePartyHPBar(i)
...
function Module.UpdatePartyHPBar(i)
    ...
    if UnitExists(pf.unit) then
        ...
        Module.SyncFadingBar(healthbar)
...
function frame:OnEvent(event, arg1, ...)
    ...
    elseif event == 'PLAYER_FOCUS_CHANGED' then
        Module.ReApplyFocusFrame()
        Module.UpdateFocusText()
        Module.ResetFadingBarsForUnit(FocusFrameHealthBar, FocusFrameManaBar)
    elseif event == 'PLAYER_ENTERING_WORLD' then
    ...
    elseif event == 'PLAYER_TARGET_CHANGED' then
        -- Module.ApplySettings()
        Module.ReApplyTargetFrame()
        Module.ResetFadingBarsForUnit(TargetFrameHealthBar, TargetFrameManaBar)
        -- Module.ChangePlayerframe()
    ...
    elseif event == 'COMBAT_LOG_EVENT_UNFILTERED' then
        Module.HandleCombatLogEvent(arg1, ...)
    end
end
